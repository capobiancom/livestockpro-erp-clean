    ...
    <Layout>
        ...
        <div class="grid grid-cols-1 md:grid-cols-4 gap-4 mb-6">
            ...
            >
                <div class="flex items-center justify-between">
                    <div>
                        <p class="text-sm text-blue-600 font-semibold">Late</p>
                    ...
                    >
                        ...
                        >
                            ...
                            <td class="px-6 py-4 whitespace-nowrap">
                                <span
                                    :class="
                                        getSourceBadgeClass(attendance.source)
                                    "
                                    class="inline-flex items-center px-3 py-1 rounded-full text-xs font-semibold capitalize"
                                >
                                    {{ attendance.source }}
            ...
            <div class="bg-white rounded-lg shadow-xl p-6 max-w-md w-full mx-4">
                ...
                <p class="text-gray-600 mb-6">
                    ...
                    <span class="font-semibold"
                        >{{ attendanceToDelete?.employee?.first_name }}
                        {{ attendanceToDelete?.employee?.last_name }} on
                        {{ attendanceToDelete?.date }}</span
...
// Compute status counts from attendances data
const statusCounts = computed(() => {
    ...
    props.attendances.data.forEach((attendance) => {
        if (counts.hasOwnProperty(attendance.status)) {
...
const formatTime = (timeString) => {
    ...
    try {
        const parsedTime = parse(timeString, "yyyy-MM-dd HH:mm:ss", new Date());
        if (isNaN(parsedTime.getTime())) {
            console.error("Invalid Date encountered for time:", timeString);
            return "Invalid Time";
        }
...
const deleteAttendance = () => {
    Inertia.delete(`/attendances/${attendanceToDelete.value.id}`, {
        onSuccess: () => {
...
const formatDate = (dateString) => {
    ...
    try {
        ...
        if (isNaN(parsedDate.getTime())) {
            console.error("Invalid Date encountered for date:", dateString);
            return "Invalid Date";
        }
        return format(parsedDate, "MMMM dd, yyyy");
    } catch (error) {
        console.error(
            "Error formatting date with date-fns for input:",
            dateString,
            error,
        );
        return "Invalid Date";
