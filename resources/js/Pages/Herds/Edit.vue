<template>
    <Layout>
        <template #title>
            <h2 class="text-3xl font-bold text-gray-800"> {{ $t('edit_herd') }} </h2>
        </template>
...
const form = useForm({
    farm_id: props.herd.farm_id || "",
    name: props.herd.name || "",
    code: props.herd.code || "",
    description: props.herd.description || "",
});

function submit() {
    form.put(`/herds/${props.herd.id}`);
}
</script>
