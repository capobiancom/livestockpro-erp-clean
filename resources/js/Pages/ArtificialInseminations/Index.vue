<template>
    <Head title="Artificial Insemination Records" />
    ...
    <Layout>
            ...
            <div class="flex items-center justify-between">
                <div>
                    ...
                    <p class="text-sm text-gray-500 mt-1">
                        Manage artificial insemination events
                    </p>
                </div>
            </div>
        </template>
                    ...
                    <div class="flex-1">
                        <div class="relative">
                            <input
                                ...
                                @input="handleSearch"
                                type="text"
                                placeholder="Search by semen batch no, breed name or animal tag..."
        ...
        <div
            ...
            class="mb-6 bg-teal-50 border-l-4 border-teal-500 p-4 rounded-lg"
        >
                ...
                <div class="flex items-center justify-between">
                    <div>
                        ...
                        <p class="text-3xl font-bold text-blue-700 mt-1">
                            {{ formatCurrency(statistics.average_cost) }}
                        </p>
                    </div>
                    <div class="bg-blue-500 rounded-full p-3">
                        ...
                        >
                            <path
                                ...
                                stroke-linejoin="round"
                                stroke-width="2"
                                d="M12 8c-1.657 0-3 .895-3 2s1.343 2 3 2 3 .895 3 2-1.343 2-3 2m0-8c1.11 0 2.08.402 2.599 1L15 4H9l-.401 1.001A5.979 5.979 0 0012 8zm0 0v8m-3-2.582V9m3 3.418V15m0 0v2.582c-1.11 0-2.08-.402-2.599-1L9 18h6l.401-1.001A5.979 5.979 0 0012 16z"
                            />
                        </svg>
                    </div>
                </div>
        ...
        <div class="bg-white rounded-lg shadow-md overflow-hidden">
            <div class="overflow-x-auto">
                <table class="w-full">
                    <thead
                        class="bg-gradient-to-r from-teal-500 to-cyan-500 text-white"
                    >
                        <tr>
                            <th
                    ...
                    <tbody class="divide-y divide-gray-200">
                        <tr
                            v-for="ai in artificialInseminations.data"
                            :key="ai.id"
                            class="hover:bg-teal-50 transition duration-150"
                        >
                            <td class="px-6 py-4">
                                <Link
                                    v-if="ai.reproduction_record?.animal"
                                    :href="
                                        route(
                                            'animals.show',
                                            ai.reproduction_record.animal.id,
                                        )
                                    "
                                    class="text-sm font-medium text-teal-600 hover:text-teal-900"
                                >
                                    <div
                                        class="text-sm font-medium text-gray-900"
                                    >
                                        {{ ai.reproduction_record?.animal.tag }}
                                    </div>
                                    <div class="text-sm text-gray-500">
                                        {{ ai.reproduction_record.animal.name }}
                                    </div>
                                </Link>
                            ...
                            <td
                                class="px-6 py-4 whitespace-nowrap text-sm text-gray-700"
                            >
                                {{ formatDate(ai.insemination_date) }}
                            </td>
                            <td
                                class="px-6 py-4 whitespace-nowrap text-sm text-gray-700"
                            >
                        ...
                        <tr v-if="!artificialInseminations.data.length">
                            <td colspan="6" class="px-6 py-12 text-center">
                                ...
                                >
                                    ...
                                    <p class="text-gray-400 text-sm mt-1">
                                        Click "Add AI Record" to create your
                                        first artificial insemination record
                                    </p>
                                </div>
                            </td>
                        </tr>
            ...
            <!-- Pagination -->
            <div
                v-if="artificialInseminations.links.length > 3"
                class="bg-gray-50 px-6 py-4 border-t border-gray-200"
            >
                <div class="flex justify-center gap-1">
                    <Link
                        v-for="(link, index) in artificialInseminations.links"
                        :key="index"
                        :href="link.url"
                        v-html="link.label"
                        :class="[
        ...
        >
            <div class="bg-white rounded-lg shadow-xl p-6 max-w-md w-full mx-4">
                ...
                <p class="text-gray-600 mb-6">
                    Are you sure you want to delete this artificial insemination
...
const props = defineProps({
    artificialInseminations: Object,
    filters: Object,
    statistics: Object, // New prop for statistics
    currency: String, // New prop for currency symbol
});
...
const formatCurrency = (value) => {
    ...
    return new Intl.NumberFormat("en-US", {
        style: "currency",
        currency: props.currency || "USD", // Use the currency prop
    }).format(value);
};
...
const confirmDelete = (ai) => {
    recordToDelete.value = ai;
    showDeleteModal.value = true;
};

const deleteRecord = () => {
    Inertia.delete(
        route("artificial-inseminations.destroy", recordToDelete.value.id),
        {
            onSuccess: () => {
