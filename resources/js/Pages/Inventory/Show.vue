...
const isLowStock = () => {
    if (props.item.quantity === null || props.item.min_quantity === null)
        return false;
    return props.item.quantity <= props.item.min_quantity;
};

const isGoodStock = () => {
    if (props.item.quantity === null || props.item.min_quantity === null)
        return true;
    return props.item.quantity > props.item.min_quantity * 2;
};
...
const getStockPercentage = () => {
    if (!props.item.min_quantity || props.item.quantity === null) return 0;
    const percentage =
        (props.item.quantity / (props.item.min_quantity * 3)) * 100;
...
const calculateTotalValue = () => {
    if (!props.item.unit_cost || !props.item.quantity) return "—";
    const total = props.item.unit_cost * props.item.quantity;
