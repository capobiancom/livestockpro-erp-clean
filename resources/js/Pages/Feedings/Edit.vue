            ...
            <div class="bg-white rounded-lg shadow-lg p-6">
                ...
                <div class="flex gap-3 mt-6">
                    ...
                    >
                        ...
                        <span v-else>Update Feeding Record</span>
                    </button>
                    <Link
...
const form = useForm({
    animal_id: props.feeding.animal_id || null,
    group_id: props.feeding.group_id || null,
    feeding_date: props.feeding.feeding_date,
    feeding_time: props.feeding.feeding_time,
    notes: props.feeding.notes || "",
    feeding_items:
        (props.feeding.feeding_items ?? props.feeding.feedingItems ?? [])
            .length > 0
            ? (props.feeding.feeding_items ?? props.feeding.feedingItems).map(
                  (item) => ({
                      id: item.id,
                      item_id: item.item_id ?? item.itemId ?? item.item?.id,
                      quantity: item.quantity,
                  }),
              )
            : [{ item_id: null, quantity: null }],
});
...
function submit() {
    form.put(`/feedings/${props.feeding.id}`);
}
</script>
