...
<head>
    ...
    <title>Cost of Feed per Cow - Print</title>
    ...
    <div class="sheet">
        ...
        <header class="topbar">
            <div class="brand">
                ...
                <div>
                    <h1 class="title">Cost of Feed per Cow</h1>
                    <p class="subtitle">
                        Total monetary value of feed consumed per cow from stock movements (FIFO-based unit cost).
                    </p>
                </div>
            </div>
            ...
            <div class="meta">
                ...
                @if(!empty($filters['animal_id']))
                    <div><b>Cow:</b> #{{ $filters['animal_id'] }}</div>
                @else
                    <div><b>Cow:</b> All</div>
                @endif
                @if(!empty($filters['q']))
                ...
                @endif
                <div><b>Sort:</b> {{ $filters['sort'] ?? 'total_cost' }} ({{ $filters['direction'] ?? 'desc' }})</div>
            </div>
        </header>
        ...
        <section class="summary">
            ...
            <div class="card">
                ...
                <div class="value num money">{{ number_format((float)($summary['total_cost'] ?? 0), 2) }}</div>
            </div>
        </section>
        ...
        <div class="table-wrap">
            <table>
                <thead>
                    <tr>
                        <th style="width: 44%;">Cow</th>
                        <th style="width: 18%;" class="text-right">Total feed</th>
                        <th style="width: 18%;" class="text-right">Avg cost / unit</th>
                ...
                <tbody>
                    @forelse($rows as $r)
                        <tr>
                            ...
                            <td class="text-right num avg">{{ number_format((float)($r['avg_cost_per_kg'] ?? 0), 2) }}</td>
                            <td class="text-right num money">{{ number_format((float)($r['total_cost'] ?? 0), 2) }}</td>
                        </tr>
                    @empty
                        <tr>
