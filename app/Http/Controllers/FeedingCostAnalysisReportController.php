...
class FeedingCostAnalysisReportController extends Controller
{
    public function print(Request $request)
    {
        ...
        $selectedItemLabel = 'All items';
        if (!empty($validated['inventory_item_id'])) {
            $selected = collect($items)->firstWhere('id', (int) $validated['inventory_item_id']);
            if ($selected) {
                $selectedItemLabel = $selected['name'] ?? $selected['sku'] ?? 'Selected item';
            } else {
                $selectedItemLabel = 'Selected item';
            }
        }

        return view('reports.feeding-cost-analysis.print', [
