...
CREATE TABLE IF NOT EXISTS `feeding_records`(
  ...
  `feeding_time` VARCHAR(255) check(`feeding_time` in('morning', 'evening')) not null,
  `animal_id` integer,
  `group_id` integer,
...
CREATE TABLE IF NOT EXISTS `breeds`(
  ...
  `farm_id` integer,
  `user_id` integer,
  `origin` VARCHAR(255) check(`origin` in('local', 'exotic', 'cross')) not null default 'local',
  `animal_type` VARCHAR(255) check(`animal_type` in('cow', 'bull')) not null default 'cow',
...
CREATE TABLE IF NOT EXISTS `milk_sales`(
  ...
  `sale_transaction_source_type` VARCHAR(255),
...
CREATE TABLE IF NOT EXISTS `pregnancies`(
  ...
  `pregnancy_status` VARCHAR(255) not null default('ongoing'),
  `health_notes` TEXT,
  `created_at` DATETIME,
  `updated_at` DATETIME,
  `deleted_at` DATETIME,
...
CREATE TABLE IF NOT EXISTS `stock_movements`(
  ...
  `item_type` VARCHAR(255) not null,
  `item_id` integer not null,
  `movement_type` VARCHAR(255) check(`movement_type` in('in', 'out', 'adjustment')) not null,
  `source_event_type` VARCHAR(255) check(`source_event_type` in('purchase', 'consumption', 'adjustment', 'transfer', 'loss', 'expired')) not null,
...
CREATE TABLE IF NOT EXISTS `settings`(
  ...
  `website_currency` VARCHAR(255),
  `super_admin_mail_mailer` VARCHAR(255),
  `super_admin_mail_host` VARCHAR(255),
  `super_admin_mail_port` integer,
  `super_admin_mail_username` VARCHAR(255),
  `super_admin_mail_password` TEXT,
  `super_admin_mail_encryption` VARCHAR(255),
  `super_admin_mail_from_address` VARCHAR(255),
  `super_admin_mail_from_name` VARCHAR(255),
  `website_logo_path` VARCHAR(255)
);
...
CREATE TABLE IF NOT EXISTS `attendances`(
  ...
  `status` VARCHAR(255) check(`status` in('present', 'absent', 'leave', 'late')) not null,
  `source` VARCHAR(255) check(`source` in('manual', 'biometric', 'mobile')) not null,
  `created_at` DATETIME,
  `updated_at` DATETIME,
  foreign key(`employee_id`) references `employees`(`id`) on delete cascade,
  foreign key(`farm_id`) references `farms`(`id`) on delete cascade,
...
CREATE TABLE IF NOT EXISTS `leave_requests`(
  ...
  `reason` TEXT,
  `status` VARCHAR(255) check(`status` in('pending', 'approved', 'rejected')) not null default 'pending',
  `approved_by` integer,
  `created_at` DATETIME,
  `updated_at` DATETIME,
  foreign key(`employee_id`) references `employees`(`id`) on delete cascade,
...
CREATE TABLE IF NOT EXISTS `chart_of_accounts`(
  ...
  `name` VARCHAR(255) not null,
  `type` VARCHAR(255) check(`type` in('asset', 'liability', 'equity', 'income', 'expense')) not null,
  `parent_id` integer,
...
CREATE TABLE IF NOT EXISTS `customers`(
  ...
  `email` VARCHAR(255),
  `address` VARCHAR(255),
  `type` VARCHAR(255) check(`type` in('milk_buyer', 'animal_buyer', 'wholesaler')) not null,
  `created_at` DATETIME,
  `updated_at` DATETIME,
...
CREATE TABLE IF NOT EXISTS `supplier_payments`(
  ...
  `purchase_source_type` VARCHAR(255),
  `purchase_source_id` integer,
  `payment_date` date not null,
  `amount` numeric not null,
  `payment_method` VARCHAR(255) check(`payment_method` in('Cash', 'Bank Transfer', 'Mobile Banking', 'Cheque')) not null,
  `reference_number` VARCHAR(255),
  `notes` TEXT,
  `farm_id` integer,
  `user_id` integer,
...
CREATE TABLE IF NOT EXISTS `cash_accounts`(
  ...
  `account_id` integer,
  `name` VARCHAR(255) not null,
  `type` VARCHAR(255) check(`type` in('cash', 'bank', 'mobile')) not null,
...
CREATE TABLE IF NOT EXISTS `cash_transactions`(
  ...
  `transaction_date` date not null,
  `amount` numeric not null,
  `direction` VARCHAR(255) check(`direction` in('in', 'out')) not null,
...
CREATE TABLE IF NOT EXISTS `fixed_assets`(
  ...
  `user_id` integer,
  `name` VARCHAR(255) not null,
  `asset_type` VARCHAR(255) check(`asset_type` in('machinery', 'shed', 'vehicle', 'equipment', 'land', 'building', 'other')) not null default 'machinery',
  `purchase_value` numeric not null,
  `purchase_date` date not null,
  `useful_life_years` integer not null,
  `depreciation_method` VARCHAR(255) check(`depreciation_method` in('straight_line')) not null default 'straight_line',
  `status` VARCHAR(255) check(`status` in('active', 'disposed', 'under_maintenance', 'sold')) not null default 'active',
